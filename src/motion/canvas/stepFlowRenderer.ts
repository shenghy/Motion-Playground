...
export const renderStepFlowToCanvas: CanvasMotionRenderer<StepFlowParams> = ({ ctx, params, localTime, resources }) => {
  ...
  const panel = { x: 122, y: 110, width: 630, height: 760 }
  drawPanel(ctx, { ...panel, fill: 'rgba(5,6,6,.66)', stroke: null })
  drawText(ctx, { text: params.eyebrow || '07 / 流程图', x: 156, y: 162, font: `500 15px ${resources.monoFont}`, color: CANVAS_COLORS.accentBlue, maxWidth: 540, alpha: state.headerOpacity })
  drawText(ctx, { text: params.title || '未命名流程', x: 156, y: 198, font: `600 44px ${resources.contentFont}`, color: CANVAS_COLORS.paper, maxWidth: 540, alpha: state.headerOpacity })
  drawPencilLine(ctx, { x1: 156, y1: 265, x2: 720, y2: 265, color: 'rgba(241,238,229,.36)', width: 2, alpha: state.headerOpacity })
  ...
  state.items.forEach((item, index) => {
    ...
    drawText(ctx, { text: item.label, x: 220, y: y - 16, font: `560 28px ${resources.contentFont}`, color: active ? CANVAS_COLORS.paper : item.completed ? '#7996b8' : '#7e8386', maxWidth: 480, alpha: item.opacity })
    drawPencilLine(ctx, { x1: 220, y1: y + 24, x2: 720, y2: y + 24, color: progressed ? CANVAS_COLORS.accentBlueMuted : '#424649', width: active ? 2 : 1, alpha: item.opacity })
  })
}
