...
export const renderProfileRevealToCanvas: CanvasMotionRenderer<ProfileRevealParams> = ({ ctx, params, localTime, resources }) => {
  ...
  drawPanel(ctx, { ...card, fill: 'rgba(8,10,10,.72)', stroke: 'rgba(241,238,229,.42)', alpha: state.card.opacity })
  ...
  drawText(ctx, { text: params.title || '未命名人物', x: 118, y: 354 + state.title.y, font: `400 48px ${resources.handwritingFont}`, color: CANVAS_COLORS.paper, maxWidth: 540, alpha: state.title.opacity })
  ...
  facts.forEach((fact, index) => {
    ...
    drawText(ctx, { text: fact.text || `信息 ${index + 1}`, x: 184, y, font: `400 29px ${resources.handwritingFont}`, color: CANVAS_COLORS.paper, maxWidth: 470, alpha: layer.opacity })
  ...
  const railX = 1668
  drawPanel(ctx, { x: railX, y: 238 + state.rail.y, width: 194, height: 584, fill: 'rgba(5,6,6,.58)', stroke: 'rgba(241,238,229,.25)', alpha: state.rail.opacity })
