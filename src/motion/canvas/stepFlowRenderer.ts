...
export const renderStepFlowToCanvas: CanvasMotionRenderer<StepFlowParams> = ({ ctx, params, localTime, resources }) => {
  ...
  drawText(ctx, { text: params.title || '未命名流程', x: 122, y: 245, font: `400 39px ${resources.handwritingFont}`, color: CANVAS_COLORS.paper, maxWidth: 470, alpha: state.headerOpacity })
  ...
  state.items.forEach((item, index) => {
    ...
    drawText(ctx, { text: item.label, x: x + 68, y: y - 16, font: `400 27px ${resources.handwritingFont}`, color: item.active > 0.5 ? CANVAS_COLORS.paper : '#7e8386', maxWidth: 310, alpha: item.opacity })
  ...
  const rx = 1670
  drawPanel(ctx, { x: rx, y: 320, width: 192, height: 306, fill: 'rgba(5,6,6,.56)', stroke: 'rgba(241,238,229,.25)' })
  ...
  drawText(ctx, { text: params.statusNote || active.label, x: rx + 18, y: 540, font: `400 18px ${resources.handwritingFont}`, color: '#9ca09f', maxWidth: 154 })
}
