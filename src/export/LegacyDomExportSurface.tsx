...
import type { MotionDefinition, ParameterValues } from '../motion/types'
...
export interface LegacyDomExportSurfaceHandle {
...
export const LegacyDomExportSurface = forwardRef<
  LegacyDomExportSurfaceHandle,
  { cards: OverlayCard[] }
>(function LegacyDomExportSurface({ cards }, ref) {
  const rootRef = useRef<HTMLDivElement>(null)
  const fontEmbedCssRef = useRef<string | null>(null)
  const [frameTime, setFrameTime] = useState<number | null>(null)
  const activeCards = frameTime === null
    ? []
    : cards
  ...
  useImperativeHandle(ref, () => ({
    async beginCaptureSession() {
      if (!rootRef.current) throw new Error('透明导出舞台尚未准备完成')
    ...
    async capturePng() {
      ...
      if (!blob) throw new Error('生成透明 PNG 帧失败')
      return blob
    },
    endCaptureSession() {
      fontEmbedCssRef.current = null
  ...
  return (
    <div className="export-surface-host" aria-hidden="true">
      <div
        ...
        data-testid="legacy-export-surface"
        data-background="transparent"
      >
        {activeCards.map((card) => {
          const definition = getMotionDefinition(card.motionId) as unknown as
            MotionDefinition<ParameterValues>
          ...
          return (
            ...
            >
              <MotionComponent params={card.params} playbackTime={localTime} />
            </div>
          )
        })}
      </div>
