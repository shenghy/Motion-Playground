...
export const renderCompareSplitToCanvas: CanvasMotionRenderer<CompareSplitParams> = ({ ctx, params, localTime, resources }) => {
  ...
  drawText(ctx, { text: params.title || '未命名对比', x: 96, y: 104, font: `600 38px ${resources.contentFont}`, color: CANVAS_COLORS.paper, maxWidth: 620, alpha: state.headerOpacity })
  ...
  const drawSide = (side: 'left' | 'right') => {
    ...
    drawPanel(ctx, { ...box, fill: emphasized ? 'rgba(20,23,23,.82)' : 'rgba(5,6,6,.62)', stroke: null })
    ...
    drawText(ctx, { text: label, x: box.x + 44, y: box.y + 144, font: `500 32px ${resources.contentFont}`, color: emphasized ? CANVAS_COLORS.paper : '#999e9f', maxWidth: box.width - 88 })
  ...
  drawPanel(ctx, { x: 82, y: 900, width: 1758, height: 96, fill: 'rgba(5,6,6,.68)', stroke: null, alpha: state.resultOpacity })
  ...
  drawText(ctx, { text: params.conclusion || '暂无结论', x: 460, y: 919, font: `500 30px ${resources.contentFont}`, color: CANVAS_COLORS.paper, maxWidth: 900, alpha: state.resultOpacity })
